...
import { FiFileText, FiCalendar, FiUser, FiClock, FiTag, FiDownload } from 'react-icons/fi';
...
const ArticlePage = ({ isDarkMode, setIsDarkMode }) => {
  ...
  // Ambil basePath dari Vite (sudah kita atur sebelumnya)
  const basename = "/health-info-website";
  
  // Perbaiki jalur gambar dan PDF agar selalu mengarah ke folder utama
  const correctedImageUrl = basename + '/' + article?.imageUrl;
  const correctedPdfUrl = basename + '/' + article?.pdfUrl;
    ...
    <div className="bg-white dark:bg-gray-950 min-h-screen font-sans transition-colors duration-500 relative">
      ...
      <div className="relative z-10">
        <Helmet>
          ...
          <meta property="og:image" content={correctedImageUrl} />
          <meta property="og:url" content={shareUrl} />
          <meta name="twitter:card" content="summary_large_image" />
          <meta name="twitter:title" content={article.title} />
          <meta name="twitter:description" content={article.description} />
          <meta name="twitter:image" content={correctedImageUrl} />
        </Helmet>
